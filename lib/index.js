...
new P5(p5 => {
  ...
  p5.setup = () => {
    const buttons = {
      cleanSlate: createElement('button', { class: 'btn', title: 'Start training from a new, untrained neural network' }, 'Clean slate'),
      preTrained: createElement('button', { class: 'btn', title: 'Start training from a pre-trained neural configuration' }, 'Pre-trained')
    }
    ...
    buttons.preTrained.addEventListener('click', (event) => {
      event.preventDefault()
      loadBest().then(() => {
        newGeneration()
        p5.loop()
