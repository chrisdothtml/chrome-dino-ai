...
import { randBoolean, random } from '../common/utils.js'

const actorSpawnRng = random.seeded('actorSpawn')
...
export default class Game {
  ...
  newGeneration () {
    ...
    if (config.settings.ai.guessEnabled) {
      for (let i = 0; i < config.settings.ai.populationSize; i++) {
        dinos.push(new Dino(config.canvas.height))
      }
    } else if (this.deadDinos.length) {
      if (config.settings.ai.geneticsEnabled) {
        const childBrains = this.naturallySelect(this.deadDinos)

        dinos = childBrains.map(brain => {
          return new Dino(config.canvas.height, this.mutateBrain(brain))
        })
      } else {
        dinos = this.deadDinos
      }
    } else if (this.bestBrain) {
      const baseBrain = this.bestBrain

      for (let i = 0; i < config.settings.ai.populationSize; i++) {
        dinos.push(
          new Dino(
            config.canvas.height,
            config.settings.ai.geneticsEnabled ? this.mutateBrain(baseBrain) : baseBrain
          )
        )
      }
    }
  ...
  spawnActor (actorType) {
    ...
    switch (actorType) {
      ...
      case 'cacti':
        ...
        if (randBoolean.call({ rng: actorSpawnRng })) {
  ...
  processNextFrame () {
    ...
    ;['birds', 'cacti', 'clouds'].forEach(key => {
      ...
      if (key === 'birds' ? this.level > 3 : true) {
        ...
        if (this.frameCount % config.settings.spawnRates[key] === 0) {
          // only spawn cactus if there aren't any birds
          const shouldSpawn = key === 'cacti' ? !this.birds.length : true
    ...
    if (this.dinos.length) {
      ...
      const stateInputs = [
        ...
        // level 8 is the hardest the difficulty ever gets
        level < 7 ? level / 8 : 1
      ]
  ...
  processNextGeneration () {
    ...
    while (this.generationIsRunning) {
