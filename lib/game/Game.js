...
export default class Game {
  ...
  processNextFrame () {
    ...
    if (this.dinos.length) {
      ...
      for (let i = dinos.length - 1; i >= 0; i--) {
        ...
        if (dino.hits([cacti[0], birds[0]])) {
          ...
          deadDinos.push(dino)
          dinos.splice(i, 1)
        } else if (config.settings.ai.rewardsEnabled) {
          dino.reward()
        }
      }
    }
    ...
    if (this.dinos.length) {
      ...
      this.lastGenerationScore = this.score
      this.newGeneration()
    }
  }

  processNextGeneration () {
    const currentGeneration = this.generations

    while (this.generations === currentGeneration) {
