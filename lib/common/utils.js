...
export function random (...args) {
  ...
  if (args.length === 2) {
    ...
    result = random.call(this) * (max - min) + min
  } else {
    result = ((this || {}).rng || Math.random)()
  }

  return result
}

random.seeded = (seedKey) => {
  if (SEED && seedrandom) {
    return random.bind({ rng: seedrandom(`${SEED}.${seedKey}`) })
  } else {
    return random
  }
}
...
export function randBoolean () {
  return Boolean(randInteger.call((this || {}), 0, 1))
}

export function randItem (arr) {
  return arr[randInteger.call((this || {}), 0, arr.length - 1)]
}
